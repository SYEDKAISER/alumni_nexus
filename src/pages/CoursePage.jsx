...
const CoursePage = () => {
  ...
  /* LOAD DEPARTMENT FROM PREVIOUS PAGE */
  ...
  /* FETCH FORM RESPONSES CSV */

  useEffect(() => {

    fetch("https://docs.google.com/forms/d/e/1FAIpQLSfRBkbeqT0XhyeLfHRKn8i7eA1Neip8Y3U63QM7g8aWv6_Lmg/viewform?usp=pp_url")
      .then(res => res.text())
      .then(text => {
    ...
    const batches =
      students
        .filter(s =>
          ...
          branchToDepartment(s.Branch)
            === department &&

          branchToCourse(s.Branch)
            === normalize(course)
  ...
  const openForm = (s) => {
    ...
    const url =
      `${base}` +
      `&entry.name=${encodeURIComponent(s.Name)}` +
      `&entry.enrollment=${encodeURIComponent(s["Enrollment No"])}` +
      `&entry.department=${encodeURIComponent(branchToDepartment(s.Branch))}` +
      `&entry.course=${encodeURIComponent(branchToCourse(s.Branch))}` +
      `&entry.batch=${encodeURIComponent(s.Batch)}`;
